...
#define RESTORE_ERROR_ARRRY_MAX		(16 * 5)
static u32 restore_len = 0;
static u8 restore_error_array[RESTORE_ERROR_ARRRY_MAX];

void restore_error_data_array(u8 *error_array)
{
	if (restore_len) {
		memcpy(error_array, restore_error_array, restore_len);
	}
}
	...
	u32 dirs)
{
	...
	if (inode == NULL) {
		printf("failed to get inode %u", inode_num);
		return EXT4_ALLOCATE_FAILED;
	}

	data = inode_allocate_data_extents(inode, len, len);
	if (data == NULL) {
		printf("failed to allocate %llu extents", len);
		return EXT4_ALLOCATE_FAILED;
	}
	...
	/* richardfeng : add_dentry here will change inqueued data block for something */
	...
	for (i = 0; i < entries; i++) {
		...
		if (offset > len || (offset == len && i != entries - 1))
			printf("internal error: dentry for %s ends at %d, past %d\n",
	...
	dentry = (struct ext4_dir_entry_2 *)(data + offset);
	dentry->inode = 0;
	/* richardfeng : dentry->rec_len here will change inqueued data block for something */
	...
	restore_len = offset + sizeof(dentry->inode) + sizeof(dentry->rec_len) + sizeof(dentry->name_len) + sizeof(dentry->file_type);
	if (restore_len > RESTORE_ERROR_ARRRY_MAX) {
		printf("\n\nrestore_error_array is too little, add it\n\n");
		restore_len = RESTORE_ERROR_ARRRY_MAX;
	}

	memset(restore_error_array, 0, RESTORE_ERROR_ARRRY_MAX);
	memcpy(restore_error_array, data, restore_len);

	/*for (aaa = 0; aaa < RESTORE_ERROR_ARRRY_MAX; aaa ++) {
		if ((aaa % 16) == 0)
			printf("\n");
		printf(" %02x", data[aaa]);
	}

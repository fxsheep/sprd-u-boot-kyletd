...
struct ext4_super_block g_ext4_super_block;
struct ext4_group_desc g_ext4_block_group;
struct ext4_inode g_root_ext4_inode;
struct ext4_inode g_ext4_inode;
unsigned char g_block_bitmap[1024];
unsigned char g_inode_bitmap[1024];
unsigned char g_buffer[1024];
unsigned char g_root_dir[1024];
unsigned char g_filename[256];
int g_blocksize;
int g_inodesz;
...
static int get_new_inode_index(unsigned char *buffer)
{
	unsigned char input;
	int operand, status;
	int count = 1;
	...
	while (*ptr == 255) {
		ptr++;
		count += 8;
		if (count > g_ext4_super_block.s_inodes_per_group)
			return -1;
	}
	printf("count = %d  *ptr = 0x%02x\n", count , *ptr);
	for (j = (count - 1) / 8; j < g_blocksize; j++) {
		input = *ptr;
		int i = 0;
		while (i <= 7) {
			operand = 1 << i;
			status = input & operand;
			printf("i = %d j = %d status = %d count = %d\n", i, j, status, count);
			if (status) {
				i++;
				count++;
			} else {
...
static int get_new_block_index(unsigned char *buffer)
{
	unsigned char input;
	int operand, status;
	int count = 1;
	...
	while (*ptr == 255) {
		ptr++;
		count += 8;
		if (count > g_ext4_super_block.s_blocks_per_group)
			return -1;
	}
	printf("count = %d  *ptr = 0x%02x\n", count , *ptr);
	for (j = (count - 1) / 8; j < g_blocksize; j++) {
...
int get_free_inode(void)
{
	...
	//printf("g_ext4_super_block.s_free_inodes_count = %d g_ext4_block_group.bg_free_inodes_count_lo = %d\n", g_ext4_super_block.free_inodes, g_ext4_block_group.bg_free_inodes_count_lo);
	if (g_ext4_super_block.s_free_inodes_count <= 0 || g_ext4_block_group.bg_free_inodes_count_lo <= 0) {
		printf("no free inode\n");
		return -1;
	}

	index = get_new_inode_index(g_inode_bitmap);
	if (index != -1) {
		g_ext4_super_block.s_free_inodes_count --;
		g_ext4_block_group.bg_free_inodes_count_lo --;
		g_ext4_block_group.bg_itable_unused_lo --;
	}
...
int get_free_block(void)
{
	...
	printf("g_ext4_super_block.s_free_blocks_count_lo = %d g_ext4_block_group.bg_free_blocks_count_lo = %d\n", g_ext4_super_block.s_free_blocks_count_lo, g_ext4_block_group.bg_free_blocks_count_lo);
	if (g_ext4_super_block.s_free_blocks_count_lo <= 0 || g_ext4_block_group.bg_free_blocks_count_lo <= 0) {
		printf("no free block\n");
		return -1;
	}

	index = get_new_block_index(g_block_bitmap);
	
	return index;
}

static int ext4fs_put_inode2root_block(int ino)
{
	struct ext2_dirent *dir;
	unsigned short offset;
	int aaa;
	
	dir = (struct ext2_dirent *)g_root_dir;
	offset = 0;
	//printf("offset = %d inode = 0x%08x direntlen = %d  namelen = %d  direntlen = %d\n", offset, dir->inode, dir->direntlen, dir->namelen, dir->direntlen);
	while (dir->inode > 0) {
	...
	dir->inode = ino;
	dir->direntlen = sizeof(struct ext2_dirent) + strlen(g_filename);
	if ((dir->direntlen % 4) != 0)
		dir->direntlen += 4 - (dir->direntlen % 4);
	dir->namelen = strlen(g_filename);
	dir->filetype = FILETYPE_REG;
	strcpy((g_root_dir + offset + sizeof(struct ext2_dirent)), g_filename);
	//printf("3offset = %d inode = 0x%08x direntlen = %d  namelen = %d  direntlen = %d\n", offset, dir->inode, dir->direntlen, dir->namelen, dir->direntlen);

	offset += dir->direntlen;
	dir = (struct ext2_dirent *)(g_root_dir + offset);
	dir->inode = 0;
	dir->direntlen = g_root_ext4_inode.i_size_lo - offset;
	dir->namelen = 0;
	dir->filetype = 0;
	//printf("4offset = %d inode = 0x%08x direntlen = %d  namelen = %d  direntlen = %d\n", offset, dir->inode, dir->direntlen, dir->namelen, dir->direntlen);

	/*for (aaa = 0; aaa < 1024; aaa ++) {
		if ((aaa % 16) == 0)
			printf("\n");
		printf(" %02x", g_root_dir[aaa]);
	}
	printf("\n");*/
	return 0;
}

static int ext4fs_read_inode_from_inodetable(int ino, struct ext4_inode *inode)
{
	int inode_per_block, offset_in_block, blockno;
	int aaa;
	int status;

	ino --;
	/* inode : 0, 1, 2, */
	inode_per_block = g_blocksize / g_ext4_super_block.s_inode_size;
	blockno = ino / inode_per_block;
	offset_in_block = ino % inode_per_block;
	printf("ino = %d inode_per_block = %d blockno = %d offset_in_block = %d\n", ino, inode_per_block, blockno, offset_in_block);

	printf("Read the inode : %d\n", ino);
	memset(g_buffer, 0, g_blocksize);
	status = ext4fs_read_block(g_ext4_block_group.bg_inode_table_lo + blockno, 0, g_blocksize, (char *)g_buffer);
	if (status == 0)
		goto fail;
	memcpy((char *)inode, (char *)(g_buffer + offset_in_block * g_ext4_super_block.s_inode_size), sizeof(struct ext4_inode));
	/*printf("\n");
	for (aaa = 0; aaa < g_blocksize; aaa ++) {
		if ((aaa % 16) == 0)
			printf("\n");
		if ((aaa % 256) == 0)
			printf("\n-------- %d ---------\n", aaa / 256);
		printf(" %02x", g_buffer[aaa]);
	}
	printf("\n");*/
...
static int ext4fs_put_inode2inodetable(int ino, struct ext4_inode *inode)
{
	int inode_per_block, offset_in_block, blockno;
	int aaa;
	int status;
	...
	printf("Write the inode : %d\n", ino);
	memset(g_buffer, 0, g_blocksize);
	status = ext4fs_read_block(g_ext4_block_group.bg_inode_table_lo + blockno, 0, g_blocksize, (char *)g_buffer);
	if (status == 0)
		goto fail;
	memcpy((char *)(g_buffer + offset_in_block * g_ext4_super_block.s_inode_size), (char *)inode, sizeof(struct ext4_inode));
	/*printf("\n");
	for (aaa = 0; aaa < g_blocksize; aaa ++) {
		if ((aaa % 16) == 0)
			printf("\n");
	...
	printf("\n");*/
	status = ext4fs_write_block(g_ext4_block_group.bg_inode_table_lo + blockno, 0, g_blocksize, (char *)g_buffer);
	if (status == 0)
		goto fail;
	return status;
fail :
...
static int ext4fs_set_block_bitmap(long int blockno, unsigned char *buffer, int blockcount)
{
	int i, remainder, status;
	unsigned char *ptr;
	unsigned char operand;
	...
	/* block : 0, 1, 2, ... ,blockno+0, blockno+1,   blockno+(blockcount-1) */
	while (blockcount > 0) {		
		i = blockno / 8;
		remainder = blockno % 8;
		//printf("blockno = %d i = %d  remainder = %d  blockcount = %d\n", blockno, i, remainder, blockcount);
		operand = 1 << remainder;
		ptr = buffer + i;
		*ptr = *ptr | operand;
		blockno ++;
		blockcount --;
		g_ext4_super_block.s_free_blocks_count_lo --;
		g_ext4_block_group.bg_free_blocks_count_lo --;
	}
...
int ext4fs_update_meta2disk(void)
{
	...
	hdr = (struct ext4_extent_header *)&(g_root_ext4_inode.i_block[0]);
	if (hdr->eh_depth != 0) {
		printf("no implement tree struction\n");
		goto fail;
	}
	
	extent = (struct ext4_extent *)&(g_root_ext4_inode.i_block[3]);
	printf("logicblock = %d  blocklen = %d  phy_block = %d\n", extent->ee_block, extent->ee_len, extent->ee_start_lo);
		
	printf("Write the root meta\n");
	status = ext4fs_write_block(extent->ee_start_lo, 0, g_root_ext4_inode.i_size_lo, (char *)g_root_dir);
	if (status == 0) {
		printf("write root meta failed\n");			
		goto fail;
	}
	printf("\n");

	printf("Write the root inode\n");
	ext4fs_put_inode2inodetable(2, &g_root_ext4_inode);
	printf("\n");

	printf("Write inode bitmap\n");
	status = ext4fs_write_block(g_ext4_block_group.bg_inode_bitmap_lo, 0, g_blocksize, (char *)g_inode_bitmap);
	if (status == 0) {
		printf("write inode bitmap failed\n");			
		goto fail;
	}
	printf("\n");

	printf("Write block group bitmap\n");
	status = ext4fs_write_block(g_ext4_block_group.bg_block_bitmap_lo, 0, g_blocksize, (char *)g_block_bitmap);
	if (status == 0) {
		printf("write block group bitmap failed\n");			
		goto fail;
	}
	printf("\n");

	printf("write the block group desc\n");
	status = ext4fs_write_block(2, 0, sizeof(struct ext4_group_desc), (char *)&g_ext4_block_group);
	if (status == 0) {
		printf("write block group desc failed\n");			
		goto fail;
	}

	printf("Write the superblock\n");
	status = ext4fs_write_block(1, 0, sizeof(struct ext4_super_block), (char *)&g_ext4_super_block);
	if (status == 0) {
		printf("write super block failed\n");			
		goto fail;
	}
...
int ext4fs_read_file_from_disk(unsigned int len, char *buf)
{
	int i, blockcnt, blockno;
	unsigned int filesize = __le32_to_cpu(g_ext4_inode.i_size_lo);
	char *point;
	int status;
	struct ext4_extent_header *hdr;
	struct ext4_extent *extent;
	...
	hdr = (struct ext4_extent_header *)&(g_ext4_inode.i_block[0]);
	if (hdr->eh_depth != 0) {
		printf("no implement tree struct\n");
		goto fail;
	}

	extent = (struct ext4_extent *)&(g_ext4_inode.i_block[3]);
	//blockcnt = (len + g_blocksize - 1) / g_blocksize;
	blockcnt = extent->ee_len;
	blockno = extent->ee_start_lo;
	printf("len = %d  blockcnt = %d blockno = %d\n", len, blockcnt, blockno);

	if(ext4fs_read_multiblock(blockno, 0, blockcnt, buf) == 0){
		goto fail;
	}
	/*
	for (i = 0; i < blockcnt; i ++) {
		point = (char *)(buf + i * g_blocksize);
		status = ext4fs_read_block(blockno + i, 0, g_blocksize, (char *)point);
		if (status == 0) {
			printf("read file data failed : %d blocks\n", i);			
			goto fail;
		}
	}*/
...
int ext4fs_write_file2disk(unsigned int len, char *buf)
{
	...
	blockcnt = (len + g_blocksize - 1) / g_blocksize;
	blockno = extent->ee_start_lo;
	printf("len = %d  blockcnt = %d  extent->ee_len = %d  blockno = %d\n", len, blockcnt, extent->ee_len, blockno);
	writen = 0;
	if(ext4fs_write_multiblock(blockno, 0, blockcnt, buf) == 0){
		goto fail;
	}
	/*
	for (i = 0; i < blockcnt; i ++) {
		if ((len - writen) >= g_blocksize) {
			point = (char *)(buf + i * g_blocksize);
			writen += g_blocksize;
		} else {
			memset(g_buffer, 0, g_blocksize);
			memcpy(g_buffer, (char *)(buf + i * g_blocksize), (len - writen));
			writen += len - writen;
			point = (char *)g_buffer;
		}
		status = ext4fs_write_block(blockno + i, 0, g_blocksize, (char *)point);
		if (status == 0) {
			printf("write file data failed : %d blocks\n", i);			
			goto fail;
		}
	}*/
...
int creat_inode_meta(int ino, int iblk, int sizebytes)
{
	...
	blks_reqd_for_file = bytes_reqd_for_file / g_blocksize;
	if ((bytes_reqd_for_file % g_blocksize) != 0)
		blks_reqd_for_file++;

	printf("total bytes for a file %u\n", blks_reqd_for_file);
	memset(&g_ext4_inode, 0, sizeof(struct ext4_inode));
	g_ext4_inode.i_mode = S_IFREG | S_IRWXU | S_IRGRP | S_IROTH | S_IXGRP | S_IXOTH;
	/* ToDo: Update correct time */
	g_ext4_inode.i_atime = timestamp;
	g_ext4_inode.i_ctime = timestamp;
	g_ext4_inode.i_mtime = timestamp;
	g_ext4_inode.i_links_count = 1;
	g_ext4_inode.i_size_lo = sizebytes;
	g_ext4_inode.i_blocks_lo = (blks_reqd_for_file * g_blocksize) / SECTOR_SIZE;
	g_ext4_inode.i_flags = EXT4_NOATIME_FL | EXT4_EXTENTS_FL;

	if (blks_reqd_for_file <= (1 << 15)) {
		hdr = (struct ext4_extent_header *)&(g_ext4_inode.i_block[0]);
		...
		extent = (struct ext4_extent *)&(g_ext4_inode.i_block[3]);
		extent->ee_block = 0;
		extent->ee_len = blks_reqd_for_file;
		extent->ee_start_hi = 0;
		extent->ee_start_lo = iblk;
		/* update g_block_bitmap */
		ext4fs_set_block_bitmap(iblk, g_block_bitmap, blks_reqd_for_file);
			
		/* put file inode to root meta buffer */
	 	ext4fs_put_inode2root_block(ino);
		/* write file inode to disk */
		ext4fs_put_inode2inodetable(ino, &g_ext4_inode);
	} else {
		printf("struct ext4_extent_idx no implement\n");
		goto fail;
	}
...
int ext4fs_filename_check(char *filename)
{
	...
	depth_dirname = zalloc(strlen(filename) + 1);
	if (!depth_dirname)
		return -ENOMEM;

	memcpy(depth_dirname, filename, strlen(filename));
	depth = find_dir_depth(depth_dirname);
	printf("depth = %d\n", depth);
	if (depth < 4) {
		printf("Please supply correct path : /partition_name/file_name\n");
		goto fail;
	}
	parse_dirname = zalloc(strlen(filename) + 1);
	if (!parse_dirname) {
		goto fail;
	}
	memcpy(parse_dirname, filename, strlen(filename));
	...
	while (dir->inode > 0) {
		...
		if (dir->namelen > 0)
			memcpy(l_name, (char *)(g_root_dir + offset + sizeof(struct ext2_dirent)), dir->namelen);
		printf("found = %d offset = %d inode = 0x%08x direntlen = %d  namelen = %d l_name = %s\n", found, offset, dir->inode, dir->direntlen, dir->namelen,  l_name);
	}
...
static int ext4fs_read_blockgroup(int group, struct ext2_block_group *blkgrp)
{
	printf("ext4fs read %d group descriptor\n",group);
}
...
int ext4fs_read_fileinode(int offset)
{
	struct ext2_dirent *dir;
	char l_name[256];
	int len, status;
	...
	printf("offset = %d inode = 0x%08x direntlen = %d  namelen = %d l_name = %s\n", offset, dir->inode, dir->direntlen, dir->namelen, l_name);

	status = ext4fs_read_inode_from_inodetable(dir->inode, &g_ext4_inode);
	if (status < 0)
		goto fail;

	len = __le32_to_cpu(g_ext4_inode.i_size_lo);
...
int ext4fs_open(const char *filename)
{
	...
	status = ext4fs_filename_check(filename);
	if (status < 0) {
		printf("%s is not exist.\n", filename);
		goto fail;
	} else {
		printf("%s is exist.\n", filename);
		status = ext4fs_read_fileinode(status);
		if (status < 0)
			goto fail;
	}
	/* file length */
...
int ext4fs_mount(char *interface, int dev, int part)
{
	...
	ext4_dev_desc = get_dev(interface, dev);
	if (ext4_dev_desc == NULL) {
		printf("Block device %s %d not supported\n", interface, dev);
		goto fail;
	}

	if (ext4fs_set_blk_dev(ext4_dev_desc, part, &info) == 0) {
		printf("set block device fail\n");
		goto fail;
	}
	...
	printf("Read the superblock\n");
	memset(&g_ext4_super_block, 0, g_blocksize);
	status = ext4fs_read_block(1, 0, sizeof(struct ext4_super_block), (char *)&g_ext4_super_block);
	printf("\n");
	point = (unsigned char *)&g_ext4_super_block;
	#ifdef DEBUG_EXT4
	for (aaa = 0; aaa < sizeof(struct ext4_super_block); aaa ++) {
		if ((aaa % 16) == 0)
			printf("\n");
	...
	if (__le16_to_cpu(g_ext4_super_block.s_magic) != EXT2_MAGIC)
		goto fail;

	if (__le32_to_cpu(g_ext4_super_block.s_rev_level == 0))
		g_inodesz = 128;
	else
		g_inodesz = __le16_to_cpu(g_ext4_super_block.s_inode_size);
	
	g_blocksize = (1 << g_ext4_super_block.s_log_block_size) * 1024;

	printf("EXT2 rev %d, inode_size %d  blocksize = %d\n", __le32_to_cpu(g_ext4_super_block.s_rev_level), g_inodesz, g_blocksize);

	printf("Read the block group desc\n");
	memset(&g_ext4_block_group, 0, g_blocksize);
	status = ext4fs_read_block(2, 0, sizeof(struct ext4_group_desc), (char *)&g_ext4_block_group);
	printf("\n");
	point = (unsigned char *)&g_ext4_block_group;
	#ifdef DEBUG_EXT4
	for (aaa = 0; aaa < sizeof(struct ext4_group_desc); aaa ++) {
		if ((aaa % 16) == 0)
			printf("\n");
		printf(" %02x ", *(point + aaa));
	}
	...
	printf("Read the block bitmap\n");
	memset(g_block_bitmap, 0, g_blocksize);
	status = ext4fs_read_block(g_ext4_block_group.bg_block_bitmap_lo, 0, g_blocksize, (char *)g_block_bitmap);
	printf("\n");
	point = (unsigned char *)g_block_bitmap;
	#ifdef DEBUG_EXT4
	for (aaa = 0; aaa < g_blocksize; aaa ++) {
		if ((aaa % 16) == 0)
			printf("\n");
	...
	printf("Read the inode bitmap\n");
	memset(g_inode_bitmap, 0, g_blocksize);
	status = ext4fs_read_block(g_ext4_block_group.bg_inode_bitmap_lo, 0, g_blocksize, (char *)g_inode_bitmap);
	printf("\n");
	point = (unsigned char *)g_inode_bitmap;
	#ifdef DEBUG_EXT4
	for (aaa = 0; aaa < g_blocksize; aaa ++) {
		if ((aaa % 16) == 0)
			printf("\n");
	...
	memset(g_buffer, 0, g_blocksize);
	status = ext4fs_read_block(g_ext4_block_group.bg_inode_table_lo, 0, g_blocksize, (char *)g_buffer);
	printf("\n");
	point = (unsigned char *)g_buffer;
	#ifdef DEBUG_EXT4
	for (aaa = 0; aaa < g_blocksize; aaa ++) {
		if ((aaa % 16) == 0)
			printf("\n");
	...
	printf("\n");
	#endif
	memset((char *)&g_root_ext4_inode, 0, g_inodesz);
	memcpy((char *)&g_root_ext4_inode, (char *)(g_buffer + g_inodesz * 1), g_inodesz);
	printf("\n");
	point = (unsigned char *)&g_root_ext4_inode;
	for (aaa = 0; aaa < g_inodesz; aaa ++) {
		if ((aaa % 16) == 0)
			printf("\n");
		printf(" %02x", *(point + aaa));
	}
	...
	printf("root length = %d\n", g_root_ext4_inode.i_size_lo);
	hdr = (struct ext4_extent_header *)&(g_root_ext4_inode.i_block[0]);
	if (hdr->eh_depth == 0) {
		extent = (struct ext4_extent *)&(g_root_ext4_inode.i_block[3]);
		printf("logicblock = %d  blocklen = %d  phy_block = %d\n", extent->ee_block, extent->ee_len, /*extent->ee_start_hi << 32 + */extent->ee_start_lo);
		
		printf("Read the root data\n");
		status = ext4fs_read_block(/*extent->ee_start_hi << 32 + */extent->ee_start_lo, 0, g_root_ext4_inode.i_size_lo, (char *)g_root_dir);
		printf("\n");
		point = (unsigned char *)g_root_dir;
		#ifdef DEBUG_EXT4
		for (aaa = 0; aaa < g_root_ext4_inode.i_size_lo; aaa ++) {
			if ((aaa % 16) == 0)
				printf("\n");
			printf(" %02x", *(point + aaa));
		}

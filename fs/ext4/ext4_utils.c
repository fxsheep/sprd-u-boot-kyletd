...
void write_ext4_image(const char *filename, int gz, int sparse, unsigned long partstart, unsigned long blocksize)
{
	int ret = 0;
	struct output_file *out;
	off_t off;
	
	out = open_output_file(filename, gz, sparse, partstart, blocksize);
	if (!out)
		return;
...
void ext4_create_resize_inode()
{
	...
	if (inode == NULL) {
		printf("failed to get resize inode");
		return;
	}
...
void ext4_create_journal_inode()
{
	...
	if (inode == NULL) {
		printf("failed to get journal inode");
		return;
	}
	...
	u8 *journal_data = inode_allocate_data_extents(inode,
			...
			info.journal_blocks * info.block_size);
	if (!journal_data) {
		printf("failed to allocate extents for journal data");
		return;
	}
...
//u64 parse_num(const char *arg)
{
	...
	u64 num = 100 /*strtoull(arg, &endptr, 10)*/;

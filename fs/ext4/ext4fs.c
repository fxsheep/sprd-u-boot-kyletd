...
int ext4fs_ls(const char *dirname)
{
	...
	status = ext4fs_filename_check(dirname);
	if (status < 0) {
...
{	
	return ext4fs_read_file_from_disk(len, buf);
}
...
{
	time_t timestamp = 0;
	int status, ino_id, iblk_id;

	/* check if the filename is already present in root */
	status = ext4fs_filename_check(fname);
	if (status < 0) {
		printf("%s is not exist, so create it\n", fname);
		ino_id = get_free_inode();
		printf("ino_id = %d\n", ino_id);
		if (ino_id == -1)
			goto fail;
		iblk_id = get_free_block();
		printf("iblk_id = %d\n", iblk_id);
		if (iblk_id == -1)
			goto fail;
		/* create file inode */
		creat_inode_meta(ino_id, iblk_id, sizebytes);
		/* write file data into disk */
		if (ext4fs_write_file2disk(sizebytes, (char *)buffer) == -1) {
			printf("Error in copying content\n");
			goto fail;
		}
		/* update super block, group block desc, block bitmap, inode bitmap, root inode and root meta buffer */
		if (ext4fs_update_meta2disk() == -1) {
			printf("Error in copying content\n");
			goto fail;
		}
	} else {
		printf("%s is exist, so write it\n", fname);
		//get_inode_meta();
	}
...
unsigned char ext4fs_data_buffer[1024];
int ext4fs_initialize_partition(unsigned char *buffer, unsigned long blockno, unsigned long blockcnt)
{
	//printf("blockno = %d  block_cnt = %d\n", blockno, blockcnt);
	if (ext4_dev_desc->block_write)
		ext4_dev_desc->block_write(ext4_dev_desc->dev, blockno, blockcnt, (unsigned long *)buffer);
...
{
	int block_no;
	unsigned char *ext4_base = (unsigned char *)EXT4_PATTERN_ADR;

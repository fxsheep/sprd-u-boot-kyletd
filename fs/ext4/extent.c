	...
	u64 backing_len)
{
	...
	/*if (data) richardfeng add : ugly?
		memset(data, 0, backing_len);*/
	...
	struct ext4_inode *inode, u64 len)
{
	...
	if (!extent_block) {
		...
		if (((int)(info.block_size - sizeof(struct ext4_extent_header) /
			...
			printf("File size %llu is too big to fit in a single extent block\n",
					len);
			return NULL;
		}
	...
	u64 backing_len)
{
	...
	if (alloc == NULL) {
		printf("failed to allocate extents for %llu bytes", len);
		return NULL;
	}
	...
	if (backing_len) {
		...
		if (!data)
			printf("failed to create backing for %llu bytes", backing_len);
	}

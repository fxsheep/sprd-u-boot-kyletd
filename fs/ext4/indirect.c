		...
		unsigned long backing_len)
{
	if (DIV_ROUND_UP(backing_len, info.block_size) > EXT4_NDIR_BLOCKS)
		printf("indirect backing larger than %d blocks", EXT4_NDIR_BLOCKS);
...
static void reserve_indirect_block(struct block_allocation *alloc, int len)
{
	if (reserve_oob_blocks(alloc, 1)) {
		printf("failed to reserve oob block");
		return;
	}

	if (advance_blocks(alloc, len)) {
		printf("failed to advance %d blocks", len);
		return;
	}
}
...
static void fill_dindirect_block(u32 *dind_block, int len, struct block_allocation *alloc)
{
	...
	for (i = 0; len >  0; i++) {
		...
		if (advance_blocks(alloc, ind_block_len)) {
			printf("failed to advance %d blocks", ind_block_len);
			return;
		}
		...
		struct block_allocation *alloc, u32 *block_len)
{
	...
	if (advance_oob_blocks(alloc, 1)) {
		printf("failed to advance oob block");
		return -1;
	}
		...
		struct block_allocation *alloc, u32 block_len)
{
	...
	if (inode_attach_direct_blocks(inode, alloc, &count)) {
		printf("failed to attach direct blocks to inode");
		return -1;
	}

	if (count > 0) {
		if (inode_attach_indirect_blocks(inode, alloc, &count)) {
			printf("failed to attach indirect blocks to inode");
			return -1;
		}
	}
	...
	if (count > 0) {
		if (inode_attach_dindirect_blocks(inode, alloc, &count)) {
			printf("failed to attach dindirect blocks to inode");
			return -1;
		}
	}
	...
	if (count > 0) {
		if (inode_attach_tindirect_blocks(inode, alloc, &count)) {
			printf("failed to attach tindirect blocks to inode");
			return -1;
		}
	}

	if (count) {
		printf("blocks left after triply-indirect allocation");
		return -1;
	}
		...
		struct ext4_inode *inode, u32 block_len)
{
	...
	if (alloc == NULL) {
		printf("Failed to allocate %d blocks", block_len + indirect_len);
		return NULL;
	}
...
void inode_allocate_indirect(struct ext4_inode *inode, unsigned long len)
{
	...
	if (alloc == NULL) {
		printf("failed to allocate extents for %lu bytes", len);
		return;
	}
	...
	if (do_inode_attach_indirect(inode, alloc, block_len))
		printf("failed to attach blocks to indirect inode");
		...
		struct block_allocation *alloc)
{
	...
	if (block_len % aux_info.bg_desc_reserve_blocks)
		printf("reserved blocks not a multiple of %d",
		...
		unsigned long backing_len)
{
	...
	if (backing_len) {
		...
		if (!data)
			printf("failed to create backing for %lu bytes", backing_len);
	}

...
void write_ext4_image(const char *filename, int gz, int sparse)
{
	 ...
	 * deal with that here.
	 */
	//printf("%s %s %d  block_size = %d\n", __FILE__, __FUNCTION__, __LINE__, info.block_size);
	if (info.block_size > 1024) {
		...
		//printf("sizeof(SUPER BLOCK) = %d\n", sizeof(struct ext4_super_block));
		if (info.block_size > 512)
			write_data_block(out, 1024, (u8*)aux_info.sb, 1024);
		else
			write_data_block(out, 512, (u8*)aux_info.sb, 512);
	}
	//printf("first_data_block = %d  block_size = %d  bg_desc_blocks = %d  sizeof(ext2_group_desc) = %d\n", aux_info.first_data_block, info.block_size, aux_info.bg_desc_blocks, sizeof(struct ext2_group_desc));
...
void ext4_create_fs_aux_info()
{
	...
	aux_info.bg_desc_blocks =
		DIV_ROUND_UP(aux_info.groups * sizeof(struct ext2_group_desc),
			info.block_size);
	printf("aux_info.bg_desc_blocks = 0x%08x\n", aux_info.bg_desc_blocks);
	...
	printf("aux_info.bg_desc_reserve_blocks = 0x%08x\n", aux_info.bg_desc_reserve_blocks);
...
void ext4_fill_in_sb()
{
	...
	for (i = 0; i < aux_info.groups; i++) {
		...
		printf("bg_inode_table%d = 0x%08x\n", i, aux_info.bg_desc[i].bg_inode_table);
...
void ext4_create_journal_inode()
{
	...
	printf("%s %s %d  sizeof(inode->i_block) = %d\n", __FILE__, __FUNCTION__, __LINE__, sizeof(inode->i_block));
	point = (unsigned char *)&inode->i_block[0];
	for (aaa = 0; aaa < sizeof(inode->i_block); aaa ++) {
		if ((aaa % 16) == 0)
			printf("\n");

	...
	struct ext4_inode *inode, u64 len)
{
	...
	printf("%s %s %d extent_block = %d  allocation_len = %d\n", __FILE__, __FUNCTION__, __LINE__, extent_block, allocation_len);
	if (!extent_block) {
	...
	for (; !last_region(alloc); extent++, get_next_region(alloc)) {
		...
		get_region(alloc, &region_block, &region_len);
		printf("%s %s %d  file_block = %d  region_len = %d  region_block = %d\n", __FILE__, __FUNCTION__, __LINE__, file_block, region_len, region_block);

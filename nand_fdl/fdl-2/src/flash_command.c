...
__align(4)
unsigned char g_fixnv_buf[FIXNV_SIZE + 4];
	...
	*(start_addr + 3) = NAND_BUS_SIZE_16;
	...
	if (0)			// for 6800h
	{
		...
		*(start_addr + 4) = NAND_MAGIC_DATA;
		...
		check_sum =
		    CheckSum((unsigned int *)(start_addr + 1),
			     (NAND_PAGE_LEN - BOOTLOADER_HEADER_OFFSET - 4));
	} else {
		if (nAdvance)
			*(start_addr + 4) = 1;
		else
...
int FDL2_DataMidst(PACKET_T * packet, void *arg)
{
	...
	if (CHECKSUM_OTHER_DATA == g_checksum) {
		if (is_nbl_write == 1)
			g_prevstatus =
			    NandWriteAndCheck((unsigned int)size,
					      (unsigned char *)(packet->
								packet_body.
								content));
		else
			g_prevstatus =
			    nand_write_fdl((unsigned int)size,
					   (unsigned char *)(packet->
							     packet_body.
							     content));
	...
	} else			//It's fixnv data. We should backup it.
	{
		memcpy((unsigned char *)g_sram_addr,
		       (char *)(packet->packet_body.content), size);
		g_sram_addr += size;
		g_status.recv_size += size;
		FDL_SendAckPacket(BSL_REP_ACK);
		return 1;
...
int FDL2_DataEnd(PACKET_T * packet, void *arg)
{
	...
	if (CHECKSUM_OTHER_DATA != g_checksum) {
		...
		fix_nv_checksum =
		    Get_CheckSum((unsigned char *)g_fixnv_buf, fix_nv_size);
		...
		while (pos < fix_nv_size) {
			...
			if (size == 0)
				break;
			if (nand_write_fdl(size, g_fixnv_buf + pos) ==
			    NAND_SUCCESS)
				g_prevstatus = NAND_SUCCESS;
			pos += size;
		}
		...
		g_fixnv_buf[FIXNV_SIZE + 0] = g_fixnv_buf[FIXNV_SIZE + 1] =
		    0x5a;
		g_fixnv_buf[FIXNV_SIZE + 2] = g_fixnv_buf[FIXNV_SIZE + 3] =
		    0x5a;
		cmd_yaffs_mount(backupfixnvpoint);
		cmd_yaffs_mwrite_file(backupfixnvfilename, (char *)g_fixnv_buf,
				      (FIXNV_SIZE + 4));
		...
		for (i = 0; i < 3; i++) {
			...
			while (pos < g_NBLFixBufDataSize) {
				...
				if (size == 0)
					break;
				if (nand_write_fdl(size, g_FixNBLBuf + pos) ==
				    NAND_SUCCESS)
					g_prevstatus = NAND_SUCCESS;
				pos += size;
			}

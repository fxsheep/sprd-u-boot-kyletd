...
void calibration_detect(int key)
{
	...
	while(!usb_serial_configed)
	{
  	     usb_gadget_handle_interrupts();
             try_to_get_caliberate_message_by_uart
        }
...
//		usb_wait_trans_done(0);
        ...
        caliberate_device = CALIBERATE_DEVICE_USB;
received_caliberation_message:
	...
	if(!caliberate_mode){
		...
		return;
              ...
              if(caliberate_device == CALIBERATE_DEVICE_USB){
        ...
        if(cmd_buf==NULL){
            ...
            return;
              ...
              if(caliberate_device == CALIBERATE_DEVICE_UART){
                   ...
                   sprintf(cmd_buf, "calibration=%d,%d,1", caliberate_mode&0xff, (caliberate_mode&(~0xff))>>8);
              }
